# 从 Kamailio 官方 GitHub 拉取 MySQL schema 到 sql/kamailio-mysql/
# PowerShell；可选环境变量 $env:KAMAILIO_RAW_URL 指定 base，默认 master

$baseUrl = if ($env:KAMAILIO_RAW_URL) { $env:KAMAILIO_RAW_URL } else { "https://raw.githubusercontent.com/kamailio/kamailio/master/utils/kamctl/mysql" }
$dest = if ($args[0]) { $args[0] } else { Join-Path $PSScriptRoot ".." "sql" "kamailio-mysql" }
